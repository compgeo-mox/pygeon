...
class P1Advection:
    ...
    def __init__(self, keyword: str) -> None:
        ...
        # Discretization of advection matrix
        self.advection_matrix_key = "advection"
    ...
    def assemble_matrix(self, g: pp.Grid, data: Dict) -> sps.csr_matrix:
        ...
        return matrix_dictionary[self.advection_matrix_key]
    ...
    def assemble_rhs(
        ...
        equation using P1 method. See self.assemble_matrix_rhs for a detaild
        description.
    ...
    def discretize(self, g: pp.Grid, data: Dict) -> None:
        ...
        if g.dim == 0:
            matrix_dictionary[self.advection_matrix_key] = sps.csr_matrix((1, 1))
            return
        ...
        for c in np.arange(g.num_cells):
            ...
            A = self.advH1(
                beta[:g.dim, c], g.cell_volumes[c], coord_loc, g.dim
            )
        ...
        matrix_dictionary[self.advection_matrix_key] = M

    # ------------------------------------------------------------------------------#

    def advH1(self, beta, c_volume, coord, dim):
        """ Compute the local advection H1 matrix using the P1 Lagrangean approach.
        ...
        M = c_volume * np.ones((dim + 1, dim + 1)) / (dim + 1)

        return np.dot(beta, dphi) * M
