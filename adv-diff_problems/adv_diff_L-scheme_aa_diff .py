...
L-scheme linearization at iteration i:
    ∇·( D(u^{i}) ∇u^{i} ) ≈ ∇·( D(u^{i-1}) ∇u^{i} ) + L (u^{i} - u^{i-1})
...
This gives the matrix equation:
    (M + Δt A + Δt D^{i-1} + L M)(u^{n}) = Δt F + Δt BC + M u^{n-1} + L M u^{i-1}
"""
...
m = 4  # Depth for Anderson acceleration
inflow_rate = 10.0  # Inflow rate for the boundary condition

grid_sizes = [[80, 80]]  # [[5, 5], [10, 10], [20, 20], [40, 40]]
dim = [1, 1]
num_step_list = [2**5]  # np.array((25, 100, 400, 1600))
end_time = 100
...
for j, (num_steps, grid_size) in enumerate(zip(num_step_list, grid_sizes)):
    ...
    # fmt: off
    global_matrix_const = mass + dt * adv + dt * L * mass
    # fmt: on
    ...
    # initialize anderson acceleration residuals
    aa_res = np.empty((m, dof_u), dtype=np.float64)
    ...
    for n in range(1, num_steps + 1):
        ...
        for i in range(iter):
            ...
            diff_cell = solver.diff_func(sd, proj_u @ u_prev)
            diff_node = solver.diff_func(sd, u_prev)
            ...
            # set iterative rhs
            rhs = rhs_fixed.copy()
            rhs += dt * L * mass @ u_prev  # + dt * nat_bc_vals @ diff_node
            ...
            u = solver.anderson_acc(u_prev, u, i)
...
print("Average # of iterations:", iter_count / num_steps)
