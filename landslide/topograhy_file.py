...
def precipitation_func(x, t):
    ...
    rain_int = 1e-7#*(x[0]>0 and x[0]<3)*(t>(0*3600))
    #p = -np.ones(np.size(x))*rain_int
    return np.array([0, -rain_int, 0])

class permeability_law:

    def __init__(self, theta_s, theta_r, alpha, n_coeff, K_s, gamma_water, gamma, cohesion, phi, Ns):
        # FLUID
        ...
        self.m_coeff     = 1 - 1/n_coeff

        # STRUCTURE
        self.porosity = theta_s
        self.gamma    = gamma 
        ...
        theta_expression = theta_r + (theta_s - theta_r) / (1 + (-alpha * psi_var) ** n_coeff) ** self.m_coeff
        effective_saturation = (theta_expression - theta_r) / (theta_s - theta_r)
        hydraulic_conductivity_expression = K_s * (effective_saturation ** .5) * ( 1 - (1 - effective_saturation ** (1 / self.m_coeff)) ** self.m_coeff ) ** 2
        ...
        self.theta_lambda = sp.lambdify(psi_var, theta_expression, 'numpy')
        self.conductivity_lambda = sp.lambdify(psi_var, hydraulic_conductivity_expression, 'numpy')
        self.DthetaDpsi_lambda = sp.lambdify(psi_var, DthetaDpsi_expression, 'numpy')
        self.DinvConductivityDpsi_lambda = sp.lambdify(psi_var, DinvConductivityDpsi_expression, 'numpy')
        self.DConductivityDpsi_expression_lambda = sp.lambdify(psi_var, DConductivityDpsi_expression, 'numpy')

        self.thr = 1e-4 

    def theta(self, psi):
        mask = np.where(psi <= -self.thr)
        res = np.ones_like(psi) * self.theta_s
        res[mask] = self.theta_lambda(psi[mask])
        return res

    def DthetaDpsi(self, psi):
        mask = np.where(psi <= -self.thr)
        res = np.ones_like(psi) * 0
        res[mask] = self.DthetaDpsi_lambda(psi[mask])
        return res

    def DinvConductivityDpsi(self, psi):
        mask = np.where(psi <= -self.thr)
        res = np.ones_like(psi) * 0
        res[mask] = self.DinvConductivityDpsi_lambda(psi[mask])
        #res[res<-1e-6] = -1e-6
        return res
    ...
    def conductivity(self, psi):
        ...
        res[mask] = self.conductivity_lambda(psi[mask])
        #res[res<] = 1 #np.maximum(conductivity_lambda(psi[mask]), 1e-3)
        return res

    def DConductivityDpsi(self, psi):
        mask = np.where(psi < 0)
        res = np.ones_like(psi) * 0
        res[mask] = self.DConductivityDpsi_expression_lambda(psi[mask])
        return res


    def plot_Van_Genuchten_func(self, is_plot_bool = False):
        ...
        if is_plot_bool:
            plt.figure()
            plt.plot(psi_plot, self.conductivity(psi_plot))
            plt.show()

            plt.figure()
            plt.plot(psi_plot, 1./self.conductivity(psi_plot))
            plt.show()

            plt.figure()
            plt.plot(psi_plot, self.DinvConductivityDpsi(psi_plot))
            plt.show()

            plt.figure()
            plt.plot(psi_plot, self.DthetaDpsi(psi_plot))
            plt.show()

            plt.figure()
            plt.plot(psi_plot, self.theta(psi_plot))
            plt.show()
...
def topography_pts(lc: Optional[float] = 0.1):
    ...
    # reale
    #df = pd.read_csv('landslide/Ville-San-Pietro/profilo.csv', sep=";", decimal=",")
    #x_vec = df[df.keys()[0]]
...
def mark_boundary_faces(subdomain, P0, RT0, initial_pressure_func, is_plot_bool = False):
    bc_value_bedrock = []
    ...
    def bc_gamma_d(x, t):
        return initial_pressure_func(bedrock_func(x))
...
def generate_msh(output_directory, mesh_size_top, mesh_size_bottom):
    folder = os.getcwd() + "/" + output_directory + "/"
    file_name = "mesh.msh"

    gmsh.initialize()

    lc = mesh_size_top
    big_lc = mesh_size_bottom
    dim = 2

    # orientazione in senso orario
    pts_topo = topography_pts(lc)
    pts_bed = bedrock_pts(big_lc)
    pts = pts_topo + pts_bed

    pts_array_bed = np.array(pts_bed)
    domain_extent_left  = np.min(pts_array_bed[:,0])
    domain_extent_right = np.max(pts_array_bed[:,0])

    create_grid(pts, folder + file_name)
    ...
    return subdomain, domain_extent_left, domain_extent_right
