...
def richards_MFEM(theta_s, theta_r, alpha, n_coeff, K_s, gamma_water, gamma, cohesion, phi, Ns, output_directory, number_nonlin_it, abs_tol, initial_pressure_func, saving_time_interval, potential_time_step, final_time, mesh_size_top = 0.05, mesh_size_bottom = 0.05, there_is_gravity = True):
    ...
    zero = call_optimizer(trial, prev[-dof_psi:], tree, domain_extent_left, domain_extent_right, PERM)
    trial = zero.x
    ...
    while current_time < final_time:
        ...
        time_rhs[-dof_psi:] += M_psi @ project_psi_to_fe( PERM.theta( proj_psi @ prev[-dof_psi:] ) ) /time_step
        ...
        for k in np.arange(number_nonlin_it):
            ...
            rhs[-dof_psi:] -= M_psi @ project_psi_to_fe( PERM.theta( proj_psi @ prev[-dof_psi:] ) ) /time_step
            
            pic_term = cp.compute_P0_mass_matrix(subdomain, PERM.DthetaDpsi(proj_psi @ prev[-dof_psi:]))
            ...
            #tic()
            Mass_u = cp.compute_RT0_mass_matrix(1./PERM.conductivity(proj_psi @ prev[-dof_psi:]))
            ...
            C_mat = cp.compute_dual_C(subdomain, prev[:dof_q], PERM.DinvConductivityDpsi(proj_psi @ prev[-dof_psi:]))
            ...
            flux_lat_bc[gamma_laterals] = normal_vector_lat*PERM.conductivity(prev[-dof_psi:][subdomain.cell_faces[gamma_laterals].nonzero()[1]]/subdomain.cell_volumes[subdomain.cell_faces[gamma_laterals].nonzero()[1]]) 
        ...
        zero = call_optimizer(trial_ini, current[-dof_psi:], tree, domain_extent_left, domain_extent_right, PERM)
        trial = zero.x
        writer_FoS.writerow([zero.fun])
