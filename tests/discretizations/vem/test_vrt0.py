...
def test0(self):
    ...
    assert np.allclose(Ml.data, M_known_data)
    assert np.allclose(Ml.indptr, M_known_indptr)
    assert np.allclose(Ml.indices, M_known_indices)
    ...
    assert np.allclose(interp_vals, interp_vals_known)
